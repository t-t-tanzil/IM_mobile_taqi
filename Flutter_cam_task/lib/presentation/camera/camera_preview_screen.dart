...
class _CameraPreviewViewState extends State<_CameraPreviewView> with WidgetsBindingObserver {
              ...
              builder: (context, state) {
                return switch (state.status) {
                  ...
                  CameraStatus.unavailable => _MessageView(
                      ...
                      message: state.errorMessage ?? 'No camera is available on this device.',
                      actionLabel: 'Retry',
                      onAction: _cameraCubit.initializeCamera,
                    ),
                  CameraStatus.error => _MessageView(
                      icon: Icons.error_outline,
                      message: state.errorMessage ?? 'The camera could not be started.',
                      actionLabel: 'Retry',
                      onAction: _cameraCubit.initializeCamera,
                    ),
      ...
      children: [
        ...
        Positioned(
          ...
          child: Column(
            ...
            children: [
              _BatchCountBadge(count: state.batchImageCount),
              if (state.batchImages.isNotEmpty) ...[
                const SizedBox(height: 8),
                BlocBuilder<UploadCubit, UploadState>(
                  bloc: _uploadCubit,
                  builder: (context, uploadState) {
                    return _AddToQueueButton(
                      imageCount: state.batchImageCount,
                      isAdding: uploadState.isAddingBatch,
                      onPressed: () => _addCurrentBatchToQueue(state.batchImages),
                    );
                  },
                ),
              ],
        ...
        Positioned(
          ...
          right: 0,
          bottom: 24,
          child: _CameraControls(state: state, cubit: _cameraCubit),
        ),
      ],
    );
  }
...
class _BatchCountBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text('$count photo${count == 1 ? '' : 's'}', style: const TextStyle(color: Colors.white)),
    );
  }
}
...
class _AddToQueueButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FilledButton.icon(
      ...
      label: Text('Add $imageCount to Pending Uploads'),
    );
  }
}
...
class _CameraControls extends StatelessWidget {
  const _CameraControls({required this.state, required this.cubit});
        ...
        if (state.maxZoom > state.minZoom)
          Padding(
            ...
            child: Slider(
              ...
              min: state.minZoom,
              max: state.maxZoom,
              onChanged: cubit.setZoomLevel,
            ),
          ),
        const SizedBox(height: 12),
        _CaptureButton(isCapturing: state.isCapturing, onPressed: cubit.captureImage),
      ],
    );
  }
}
...
class _ZoomButton extends StatelessWidget {
  const _ZoomButton({required this.zoom, required this.isSelected, required this.onTap});
  ...
  @override
  Widget build(BuildContext context) {
    final label = zoom % 1 == 0 ? zoom.toStringAsFixed(0) : zoom.toStringAsFixed(1);
    return Padding(
      ...
      child: GestureDetector(
        ...
        child: CircleAvatar(
          radius: 18,
          backgroundColor: isSelected ? Colors.white : Colors.black45,
          child: Text(
            '${label}x',
            style: TextStyle(
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Container(
          decoration: BoxDecoration(shape: BoxShape.circle, color: isCapturing ? Colors.grey : Colors.white),
          child: isCapturing
